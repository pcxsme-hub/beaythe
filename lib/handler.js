...
function adminUiPage() { return new Response(ADMIN_UI_HTML, { headers: { 'content-type': 'text/html; charset=utf-8' } }); }

// =================== PROXY (Worker na frente do Pages) + injeção ===================
const PAGES_ORIGIN = 'https://beauthe.pages.dev';
const MENU_INJECT = `<script>(function(){function openU(){if(document.getElementById('mi-ov'))return;var ov=document.createElement('div');ov.id='mi-ov';ov.style.cssText='position:fixed;inset:0;z-index:2147483646;background:rgba(0,0,0,.45);display:flex;flex-direction:column';var bar=document.createElement('div');bar.style.cssText='background:#2C2826;color:#fff;padding:11px 18px;display:flex;justify-content:space-between;align-items:center;font:600 14px system-ui,sans-serif';var ti=document.createElement('span');ti.textContent='Gestão de Usuários';var x=document.createElement('button');x.textContent='\\u2715 Fechar';x.style.cssText='background:transparent;border:1px solid rgba(255,255,255,.4);color:#fff;border-radius:8px;padding:5px 12px;cursor:pointer;font:13px system-ui';x.onclick=function(){ov.remove()};bar.appendChild(ti);bar.appendChild(x);var fr=document.createElement('iframe');fr.src='/api/admin/ui';fr.style.cssText='flex:1;width:100%;border:0;background:#FCFAF8';ov.appendChild(bar);ov.appendChild(fr);document.body.appendChild(ov)}function inject(){try{if(document.getElementById('mi-usuarios'))return;var ref=document.querySelector('a[href="/admin-core-sys/marketing"]')||document.querySelector('a[href^="/admin-core-sys/"]:not([href="/admin-core-sys"])');if(!ref)return;var c=ref.cloneNode(true);c.id='mi-usuarios';c.setAttribute('href','#');c.style.cursor='pointer';var wk=document.createTreeWalker(c,NodeFilter.SHOW_TEXT);var t;while(t=wk.nextNode()){if(t.nodeValue&&t.nodeValue.trim()){t.nodeValue='Usuários';break}}ref.parentNode.insertBefore(c,ref.nextSibling)}catch(e){}}document.addEventListener('click',function(e){var el=e.target&&e.target.closest&&e.target.closest('#mi-usuarios');if(el){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();openU()}},true);function boot(){inject();try{var mo=new MutationObserver(function(){if(!document.getElementById('mi-usuarios'))inject()});mo.observe(document.body,{childList:true,subtree:true})}catch(e){}}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot()})();</script>`;
function buildInjection(env, pathname) {
  let s = '';
  if (env.CLARITY_ID) s += '<script type="text/javascript">(function(c,l,a,r,i,t,y){c[a]=c[a]||function(){(c[a].q=c[a].q||[]).push(arguments)};t=l.createElement(r);t.async=1;t.src="https://www.clarity.ms/tag/"+i;y=l.getElementsByTagName(r)[0];y.parentNode.insertBefore(t,y)})(window,document,"clarity","script",' + JSON.stringify(env.CLARITY_ID) + ');</script>';
...
export async function handle(request, env) {
  ...
  try {
    ...
    if (path === '/admin/ui' && m === 'GET') return adminUiPage();
