...
function analyticsUiPage(req) {
  ...
  return new Response(ANALYTICS_UI_HTML, { headers: { 'content-type': 'text/html; charset=utf-8', 'x-frame-options': 'SAMEORIGIN' } });
}
...
async function checkoutCreateSession(env, req) {
  ...
  const shipping = 0; // Envío gratis (coincide con la UI). Cambiar aquí si se cobra envío.
  ...
  const payload = {
    mode: 'payment',
    success_url: `${site}/checkout/success?order=${code}`,
  ...
  return json({ url: session.url, order_code: code });
}
