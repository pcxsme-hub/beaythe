...
async function settingsGet(env) {
  ...
  return json({ ...s, margin: s.profit_margin_percent, shipping: s.fixed_shipping_cost });
}
...
const DROPEA_COSTS_Q = `query getCosts($id:[Int],$limit:Int){products(id:$id,limit:$limit){data{id cost_price pvpr}}}`;
...
async function adminPriceAudit(env) {
  ...
  // Mapa dropea_id -> custo: busca SÓ os ids da loja na Dropea, em lotes paralelos (o catálogo todo é grande demais).
  const costMap = new Map();
  let dropeaOk = false, dropeaErr = null;
  if (!env.DROPEA_API_KEY) dropeaErr = 'DROPEA_API_KEY não configurada no Worker.';
  else {
    const ids = [...new Set(products.map(p => p.dropea_id).filter(x => x != null).map(x => parseInt(x, 10)).filter(x => !isNaN(x)))];
    const batches = [];
    for (let i = 0; i < ids.length; i += 50) batches.push(ids.slice(i, i + 50));
    const results = await Promise.all(batches.map(b => dropeaGQL(env, DROPEA_COSTS_Q, { id: b, limit: b.length })));
    for (const res of results) {
      if (!res.ok) { dropeaErr = res.error || 'Falha ao consultar a Dropea.'; continue; }
      dropeaOk = true;
      for (const it of (res.items || [])) costMap.set(String(it.id), parseFloat(it.cost_price ?? it.pvpr) || 0);
    }
  }
  const rows = [];
...
async function adminReprice(env) {
  ...
  const costMap = new Map();
  const ids = [...new Set(products.map(x => x.dropea_id).filter(x => x != null).map(x => parseInt(x, 10)).filter(x => !isNaN(x)))];
  const batches = [];
  for (let i = 0; i < ids.length; i += 50) batches.push(ids.slice(i, i + 50));
  const results = await Promise.all(batches.map(b => dropeaGQL(env, DROPEA_COSTS_Q, { id: b, limit: b.length })));
  for (const res of results) {
